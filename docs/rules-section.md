...
The pf-handoff/pf-resume skills refer to this section as "§13" — if your numbering differs, feel free to change the number; the references stay meaningful. Mentions of the "journal" and "task packets/tickets" come from the owner's file-based protocol; if you don't have those, read them as "project notes" and "your task tracker". The skill instructions are written in English; the origin system's working chat language is Russian, and the skills keep the artifact templates (HANDOFF sections) in that language — replace them with yours if needed.
