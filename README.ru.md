...
**Опциональный компаньон — pf-workflow** (конвейер задач pf-* автора: спека → тикеты → исполнители → ревью). Если установлен и он, инструменты сцепляются автоматически, без настройки: HANDOFF-файлы ссылаются на тикеты в поле `task:`, контракт исполнителя проверяет активный HANDOFF перед стартом, смена курса обновляет список «Не делать», а ретро просматривает лог компактов. Без pf-workflow ничего не теряется — pf-handoff просто работает сам.
